...
float beta = 0.86602540378 * 3.14159265358979323846f * (60.0f / 180.0f);  // compute beta
float GyroMeasDrift = 3.14159265358979323846f * (1.0f / 180.0f);      // gyroscope measurement drift in rad/s/s (start at 0.0 deg/s/s)
float zeta = 0.86602540378 * 3.14159265358979323846f * (1.0f / 180.0f);;  // compute zeta, the other free parameter in the Madgwick scheme usually set to a small or zero value
...
// for at least the counter duration
  ...
  osDelay(0.1);  // Add delay for accumulation of samples
...
{  
 ...
 // wake up device
  ...
  osDelay(0.1); // Delay 100 ms for PLL to get established on x-axis gyro; should check for PLL ready interrupt  
...
// Configure FIFO to capture accelerometer and gyro data for bias calculation
  ...
  osDelay(0.08); // accumulate 80 samples in 80 milliseconds = 960 bytes
...
void MPU6050SelfTest(float * destination) // Should return percent deviation from factory trim values, +/- 14 or less deviation is a pass
{
   ...
   osDelay(0.25);  // Delay a while to let the device execute the self-test
